...
- 1.2.8.5. **Conectivos:** excluir tokens classificados como conectivos do protocolo (lista usada na implementação: `de`, `da`, `do`, `das`, `dos`, `e`, `ou`, `em`, `sobre`, … — ver `LEXICO_CONNECTIVOS_FIXOS` em `apps/core/code_name_connectives.py`, importada por `alias_lexico_infer.py`).
...
Dois níveis hierárquicos com o **mesmo** `receita_nome` canônico (ex.: mesmo texto «Contribuição para Fundos de Assistência Médica - Bombeiros Militares» em pai e filho).

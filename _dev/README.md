...
| ----------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Catálogo (manifest)** | Chave em `manifest.yaml` → `entries`  | Baseline sync (`spec_agents`, `spec_commits`, …) — **não editar** para regras locais (**AGENTS-10**) |
| **Repositório**         | `spec_<nome-repo>.md` (opcional)      | Especificidades **SDD** deste repo — criar quando houver overrides (**AGENTS-10**)                   |
...
Tabela completa: [`spec_agents.md`](spec_agents.md) § **1.5** (**AGENTS-05**).
...
| [`spec_context.md`](spec_context.md)         | Contexto SDD compilado (`_dev/.sdd-context.yaml`), esquema v1, **AGENTS-22** |

...
| Salto de nível | `LP < L_filho − 1` (pai **não** é o nível imediatamente abaixo do filho). |
| Radical (intermediários) | Primeiros `sum(mask[0:LP])` **dígitos** do `receita_cod` do pai (apenas caracteres numéricos; o BD armazena sem pontuação de máscara). |
...
Quando `parent_n < nivel_n − 1`, os segmentos do **filho** nos índices `LP .. L−2` devem ser **zeros canónicos**; caso contrário, `ValidationError` (mensagem sobre detalhamento nos níveis entre pai e filho).
...
|-----------|----------------------|-----|
| `parent_item_id` | Valor do campo pai (PK) | Carrega `ItemClassificacao` pai. |
...
- `parent_n >= child_nivel_numero − 1` (não há “faixa” estrita entre pai e filho para esta função).
...
- `radical = p_digits[:radical_len]`; se o pai tiver menos dígitos que `radical_len`, retorna vazio.
...
**Problema anterior:** união do intervalo do formulário com o do pai alargava a janela e podia incluir vigências irrelevantes para o **item filho que se pretende criar**.
...
**Exceção mantida:** o **fallback da máscara** com vigência do pai (secção 2 acima) continua só para obter `mask` quando a vigência do form não resolve `estrutura_codigo`; **não** expande o intervalo usado nos filtros `data_vigencia_*` da query.

### 2. `classificacao_pk` na busca = **PK do formulário**

**Problema observado:** em base de teste, o pai (ex.: PK de classificação `5`) e um item intermediário desejado para o exemplo (ex.: `item_ref` 3966 com `classificacao_id` `6`) não coincidem; filtrar pela classificação do pai zerava a lista mesmo com radical, nível e vigência corretos para o exemplo.
...
**Risco consciente:** se o utilizador escolher no form uma classificação **diferente** da do pai, a lista reflete o form (pode ser vazia ou incoerente com o vínculo real). Em produção, pai e filho devem pertencer à **mesma** classificação (`spec_parent_item_id.md`); o caso acima foi guiado por **seed / teste** com PKs distintos.
...
- Título: **“Atenção!”**
- Primeiro parágrafo: `O item pai, (` + link com código mascarado do pai + ` - ` + rótulo semântico do nível do pai + `)` + texto fixo até o código/nível do **filho** entre parêntesis (filho **sem** link no trecho principal).
...
- **`_dev/spec_parent_item_id.md`:** regras normativas de `parent_item_id`, prefixo, cauda do pai, salto e zeros no filho, mesma classificação, vigência do pai contendo a do filho, etc.
...
Linha de referência em `docs/assets/seed_item_classificacao.csv` (ex.: item com `receita_cod` `1112529000000`, nível 6, classificação MG): usado para validar que a listagem precisava do **mesmo** `classificacao_id` que o formulário quando o pai do teste apontava para outro PK de classificação.

# Navegação estrutural na change (`receita_cod`)

Quatro protocolos de navegação na view **change** de `ItemClassificacao`: percorrer códigos com vigência compatível sem alterar o registro aberto. **Não substitui** navegação por **blur** (**ITEMEC**) nem sugestão de filho (**ITEMCF**).

## Objetivo

Na **change**, permitir deslocamento estrutural entre registros navegáveis (**T-cod.2** / **ITEMEC**) via barra **<< < > >>**, endpoint `resolve-structural-navigation` e redirecionamento para a `change` do PK desempatado.

Premissa: `classification_item_structural_navigation.py`, `change_form.html` (`core-structural-nav-toolbar`), `resolve-structural-navigation/`.

## Referências

- `apps/core/classification_item_structural_navigation.py`
- `apps/core/classification_item_code_lookup.py`
- `apps/core/templates/admin/core/change_form.html`
- [`spec_itemClassificacao_editar_codigo.md`](spec_itemClassificacao_editar_codigo.md) — **ITEMEC**; **(T6)**, **(T7)**, **V1**, desempate
- [`spec_itemClassificacao_criar_filho.md`](spec_itemClassificacao_criar_filho.md) — **ITEMCF**; `__coreConfirmUnsavedIfDirty`
- [`spec_itemClassificacao_formulario.md`](spec_itemClassificacao_formulario.md) — **ITEMFORM**; `_changelist_filters`
- [`spec_itemClassificacao_mascara_apresentacao.md`](spec_itemClassificacao_mascara_apresentacao.md) — **ITEMMASK**; máscara na change
- [`spec_itemClassificacao_regras_hierarquia.md`](spec_itemClassificacao_regras_hierarquia.md) — **ITEMRH**
- [`spec_classificador-receita.md`](spec_classificador-receita.md) § **Convenções** — prefixo `ITEMNAV`
...
| Legado                                          | ID atual                  |
| ----------------------------------------------- | ------------------------- |
| `(G-nav.next-code)`                             | `ITEMNAV-01`              |
| `(G-nav.next-level)`                            | `ITEMNAV-02`              |
| `(G-nav.prev-code)`                             | `ITEMNAV-03`              |
| `(G-nav.prev-level)`                            | `ITEMNAV-04`              |
| `COD-EDIT`, `NV-EDIT`, `NV-TARGET`, `MASK-EDIT` | `ITEMNAV-10`–`ITEMNAV-13` |
| `(G-nav.level-seq.*)`                           | `ITEMNAV-30`–`ITEMNAV-33` |
| `(G-nav.ui-toolbar.*)`                          | `ITEMNAV-40`              |
| `(G-nav.ui-controls)`                           | `ITEMNAV-41`              |
| `(G-nav.ui-unsaved)`                            | `ITEMNAV-42`              |
| `(G-nav.ui-warning-class)`                      | `ITEMNAV-43`              |
| Endpoint `resolve-structural-navigation`        | `ITEMNAV-50`              |
...
| **ID normativo**   | `ITEMNAV-NN` — citação estável.                                      |
| **Prefixo**        | `ITEMNAV` — ver § **Convenções** em `spec_classificador-receita.md`. |

**Índice de IDs normativos deste arquivo:**

| ID          | Tema      | Seção | Resumo                                                                          |
| ----------- | --------- | ----- | ------------------------------------------------------------------------------- |
| ITEMNAV-01  | Protocolo | 10    | **(G-nav.next-code)** — próximo código na árvore (etapas 1–3).                  |
| ITEMNAV-02  | Protocolo | 11    | **(G-nav.next-level)** — próximo irmão / sequência L1–L3.                       |
| ITEMNAV-03  | Protocolo | 13    | **(G-nav.prev-code)** — código anterior (etapas 1–3).                           |
| ITEMNAV-04  | Protocolo | 12    | **(G-nav.prev-level)** — nível anterior (espelho de **ITEMNAV-02**).            |
| ITEMNAV-10  | Notação   | 2     | **COD-EDIT** — código canônico do registro em edição.                           |
| ITEMNAV-11  | Notação   | 4     | **NV-EDIT** — último nível com segmento ≠ zero canônico.                        |
| ITEMNAV-12  | Notação   | 5     | **NV-TARGET** — nível fixo na sequência de nível.                               |
| ITEMNAV-13  | Notação   | 4     | **MASK-EDIT** — máscara efetiva na change para segmentação.                     |
| ITEMNAV-14  | Notação   | 2     | **V1** — vigência do `instance` na change.                                      |
| ITEMNAV-15  | Notação   | 4     | **RAD-BASE** — prefixo estrutural nas etapas-1.                                 |
| ITEMNAV-20  | Regra     | 3     | Candidato navegável = **(T-cod.2)** vs **V1**; desempate bitemporal.            |
| ITEMNAV-21  | Regra     | 3     | Navegação entre `classificacao_id` — intencional; aviso **ITEMNAV-43**.         |
| ITEMNAV-30  | Sequência | 5     | **(G-nav.level-seq.1)** — início: `NV-TARGET := NV-EDIT`.                       |
| ITEMNAV-31  | Sequência | 5     | **(G-nav.level-seq.2)** — manutenção de **NV-TARGET** durante sequência.        |
| ITEMNAV-32  | Sequência | 5     | **(G-nav.level-seq.3)** — fim da sequência (código / blur / URL sem parâmetro). |
| ITEMNAV-33  | Sequência | 5     | **(G-nav.level-seq.4)** — botões código não propagam **NV-TARGET**.             |
| ITEMNAV-40  | UI        | 7     | **(G-nav.ui-toolbar)** — posição entre título e `receita_cod`.                  |
| ITEMNAV-41  | UI        | 7     | **(G-nav.ui-controls)** — quatro botões `<< < > >>`.                            |
| ITEMNAV-42  | UI        | 7     | **(G-nav.ui-unsaved)** — `__coreConfirmUnsavedIfDirty` antes de redirect.       |
| ITEMNAV-43  | UI        | 7     | **(G-nav.ui-warning-class)** — modal se classificação do destino divergir.      |
| ITEMNAV-50  | API       | 8     | `GET resolve-structural-navigation/` — params e shape JSON.                     |
| ITEMNAV-100 | Teste     | 14    | Casos manuais § **14**.                                                         |

**Índice por tema:**

| Tema       | IDs                     |
| ---------- | ----------------------- |
| Protocolos | ITEMNAV-01 … ITEMNAV-04 |
| Notação    | ITEMNAV-10 … ITEMNAV-15 |
| Regras     | ITEMNAV-20, ITEMNAV-21  |
| Sequência  | ITEMNAV-30 … ITEMNAV-33 |
| UI         | ITEMNAV-40 … ITEMNAV-43 |
| API        | ITEMNAV-50              |
| Teste      | ITEMNAV-100             |
...
| Barra **ITEMNAV-40** (**(G-nav.ui-toolbar)**) e quatro botões **ITEMNAV-41** (**(G-nav.ui-controls)**) na change | Navegação por **blur** / modais **M2–M4** (`editar_codigo`)                                          |
...
| **NV-TARGET** e sequência de nível (**(G-nav.level-seq)**)                                                       | Alterar algoritmo de **ITEMNAV-01** (**(G-nav.next-code)**) / **ITEMNAV-03** (**(G-nav.prev-code)**) |
...
| ID             | Nome            |
| -------------- | --------------- |
| **ITEMNAV-01** | Próximo código  |
| **ITEMNAV-02** | Próximo nível   |
| **ITEMNAV-03** | Código anterior |
| **ITEMNAV-04** | Nível anterior  |

### Specs relacionadas

| Spec                                                                                               | Relação                                           |
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| [`spec_itemClassificacao_editar_codigo.md`](spec_itemClassificacao_editar_codigo.md)               | **ITEMEC**; blur distinto de navegação estrutural |
| [`spec_itemClassificacao_criar_filho.md`](spec_itemClassificacao_criar_filho.md)                   | **ITEMCF**; alterações não guardadas              |
| [`spec_itemClassificacao_formulario.md`](spec_itemClassificacao_formulario.md)                     | `_changelist_filters` no redirect                 |
| [`spec_itemClassificacao_mascara_apresentacao.md`](spec_itemClassificacao_mascara_apresentacao.md) | **ITEMMASK** / **MASK-EDIT**                      |
| [`spec_itemClassificacao_regras_hierarquia.md`](spec_itemClassificacao_regras_hierarquia.md)       | **ITEMRH**; zeros canônicos                       |

## 2. Notação canónica (**ITEMNAV-10**–**ITEMNAV-14**)

| Símbolo                        | Significado                                                                                                                                                                                                                                                      |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **COD-EDIT** (**ITEMNAV-10**)  | Código canônico do registro em edição (dígitos `0-9`, sem pontuação de máscara). Equivalente a **COD-1** em `editar_codigo` na carga da página (não o valor eventualmente digitado no input).                                                                    |
| **NV-EDIT** (**ITEMNAV-11**)   | Nível hierárquico derivado de **COD-EDIT** (ver abaixo).                                                                                                                                                                                                         |
| **NV-TARGET** (**ITEMNAV-12**) | Nível de referência para **ITEMNAV-02** (**(G-nav.next-level)**) e **ITEMNAV-04** (**(G-nav.prev-level)**) durante uma **sequência de nível** (ver abaixo). Fixado no primeiro clique em próximo/nível anterior da sequência; não muda até a sequência encerrar. |
| **V1** (**ITEMNAV-14**)        | Vigência do registro aberto: `data_vigencia_inicio` e `data_vigencia_fim` do `instance` na change.                                                                                                                                                               |
| **MASK-EDIT** (**ITEMNAV-13**) | Máscara de segmentos (larguras por nível) da `estrutura_codigo` **efetivamente utilizada** para formatar/apresentar **COD-EDIT** na change ao iniciar a navegação (mesma fonte que `runCodeDigitValidation` / máscara visível no campo).                         |
...
## 3. Terminologia e candidatos (**ITEMNAV-20**–**ITEMNAV-21**)

Os protocolos retornam o candidato navegável **estruturalmente mais próximo** de **COD-EDIT**, segundo a estratégia de cada protocolo (**ITEMNAV-01**–**ITEMNAV-04**), e redirecionam para a **change** do PK desempatado.

Definições em [`spec_itemClassificacao_editar_codigo.md`](spec_itemClassificacao_editar_codigo.md) aplicam-se com **COD-EDIT** no papel de código avaliado e **V1** do registro aberto:
...
**ITEMNAV-20 — Candidato navegável:** registro **(T6)** cujo `receita_cod` satisfaz **(T-cod.2)** para **V1** do registro em edição. Todos os protocolos consideram **somente** candidatos navegáveis. Códigos inexistentes ou só com vigência diversa **não** são destino; o controle correspondente permanece desabilitado ou inativo.
...
**ITEMNAV-21 — Navegação entre classificações:** os protocolos **podem** retornar destino com `classificacao_id` diferente do registro aberto, desde que o candidato seja navegável (**(T-cod.2)** vs **V1**). É comportamento **intencional** desta funcionalidade (percorrer classificações estruturalmente compatíveis sob a mesma **MASK-EDIT**). A UI **deve** exibir aviso quando a classificação do destino divergir da do registro aberto (ver **ITEMNAV-43** (**(G-nav.ui-warning-class)**)).

---

## 4. Derivação **NV-EDIT**, **MASK-EDIT**, **RAD-BASE** (**ITEMNAV-11**–**ITEMNAV-13**, **ITEMNAV-15**)
...
### **MASK-EDIT** (**ITEMNAV-13**) (estrutura dos candidatos)
...
### **RAD-BASE** (**ITEMNAV-15**) (prefixo estrutural nas etapas-1)
...
## 5. Sequência de nível (**ITEMNAV-30**–**ITEMNAV-33**)

Protocolos **ITEMNAV-02** (**(G-nav.next-level)**) e **ITEMNAV-04** (**(G-nav.prev-level)**) percorrem a árvore preferindo **irmãos no mesmo nível hierárquico**. Quando não há mais irmãos e o protocolo **sobe** para um ramo ancestral, o destino pode ficar numa **matriz** com **NV-EDIT** menor que o nível em que o usuário iniciou a sequência. Neste caso, cliques seguintes devem **re-descer** no novo ramo (como **ITEMNAV-01** (**(G-nav.next-code)**) / **ITEMNAV-03** (**(G-nav.prev-code)**)), **um passo por clique**, até **NV-EDIT = NV-TARGET**, e só então retomar irmãos nessa profundidade.

### **NV-TARGET**

| Regra                                    | Descrição                                                                                                                                                                                                                                   |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ITEMNAV-30** (**(G-nav.level-seq.1)**) | **Início:** no **primeiro** clique em **ITEMNAV-02** (**(G-nav.next-level)**) ou **ITEMNAV-04** (**(G-nav.prev-level)**) após abrir a change **sem** sequência activa → `NV-TARGET := NV-EDIT` do registro nesse momento.                   |
| **ITEMNAV-31** (**(G-nav.level-seq.2)**) | **Manutenção:** enquanto a sequência estiver activa, **NV-TARGET** **não** muda, ainda que **NV-EDIT** do registro aberto varie após redirecionamentos.                                                                                     |
| **ITEMNAV-32** (**(G-nav.level-seq.3)**) | **Fim:** clique em **ITEMNAV-01** (**(G-nav.next-code)**) ou **ITEMNAV-03** (**(G-nav.prev-code)**); navegação por **blur** (**G-cod.blur**); abrir change **sem** parâmetro de sequência; ou URL de change que não preserve **NV-TARGET**. |
| **ITEMNAV-33** (**(G-nav.level-seq.4)**) | **ITEMNAV-01** (**(G-nav.next-code)**) e **ITEMNAV-03** (**(G-nav.prev-code)**) **não** recebem nem propagam **NV-TARGET**; encerram qualquer sequência activa.                                                                             |
...
- Na carga da change, se `structural_nav_nv_target` estiver presente e for inteiro ≥ 1, a sequência considera-se **activa** com esse **NV-TARGET** (não redefinir no primeiro clique de nível até **ITEMNAV-32** (**(G-nav.level-seq.3)**)).
- **Disponibilidade dos botões:** avaliar **ITEMNAV-02** (**(G-nav.next-level)**) / **ITEMNAV-04** (**(G-nav.prev-level)**) com **NV-TARGET** da sequência (activa ou hipotética `NV-TARGET := NV-EDIT` na pré-visualização quando inactiva).

### Reutilização de **ITEMNAV-01** (**(G-nav.next-code)**) / **ITEMNAV-03** (**(G-nav.prev-code)**)

Nas fases de **descida** / **subida** de profundidade (ver **Fase L3** abaixo), aplica-se o resolvedor de **ITEMNAV-01** (**(G-nav.next-code)**) ou **ITEMNAV-03** (**(G-nav.prev-code)**) **exactamente uma vez** por clique — o mesmo algoritmo das três etapas de cada protocolo de código, produzindo **um** candidato. **Não** confundir com o botão «Próximo código»: é reutilização do **algoritmo**, não do eixo de navegação nem da sequência de nível.

### Casos de borda **(G-nav.level-seq)**

| Situação                                                                                         | Comportamento                                                                                                                     |
| ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| L3 sem candidato **ITEMNAV-01** (**(G-nav.next-code)**) / **ITEMNAV-03** (**(G-nav.prev-code)**) | Botão de nível desabilitado; **não** saltar silenciosamente para outro ramo.                                                      |
| **NV-EDIT = NV-TARGET** após L2                                                                  | Próximo clique usa L1 (irmãos no nível alvo).                                                                                     |
| Código «não navegável» na descida L3                                                             | Ignorar; aplicar **ITEMNAV-01** (**(G-nav.next-code)**) ao conjunto restante (equivalente a candidato inexistente naquele passo). |
...
### 6.1 Nomes dos códigos encontrados

| Protocolo                               | Código encontrado | Nível do destino |
| --------------------------------------- | ----------------- | ---------------- |
| **ITEMNAV-01** (**(G-nav.next-code)**)  | `COD-PROX`        | `NV-COD-PROX`    |
| **ITEMNAV-02** (**(G-nav.next-level)**) | `COD-PROX-NV`     | `NV-COD-PROX-NV` |
| **ITEMNAV-03** (**(G-nav.prev-code)**)  | `COD-ANT`         | `NV-COD-ANT`     |
| **ITEMNAV-04** (**(G-nav.prev-level)**) | `COD-NV-ANT`      | `NV-COD-NV-ANT`  |

`NV-*` do destino: **NV-EDIT** calculado para o `receita_cod` destino com a mesma **MASK-EDIT**.

---

## 7. Interface (**ITEMNAV-40**–**ITEMNAV-43**)

### **ITEMNAV-40** (**(G-nav.ui-toolbar)**) Posição no layout

Barra de navegação estrutural **somente** na view **change** de `ItemClassificacao`, **não** na add.

| Regra                                       | Descrição                                                                                                                                                                                                                                            |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ITEMNAV-40.1** (**(G-nav.ui-toolbar.1)**) | Inserir uma faixa horizontal **entre** o cabeçalho do objeto (título `h1` com código e nome do registro) e o primeiro `.form-row` do formulário (campo `receita_cod`).                                                                               |
| **ITEMNAV-40.2** (**(G-nav.ui-toolbar.2)**) | Na **mesma faixa vertical** que `ul.object-tools` («+ Criar Código Filho», «Histórico»): alinhar a barra de navegação **à esquerda** e manter `object-tools` **à direita** (layout `flex`, `justify-content: space-between`, `align-items: center`). |
| **ITEMNAV-40.3** (**(G-nav.ui-toolbar.3)**) | **Não** colocar os controlos dentro de `ul.object-tools`, na linha do input `receita_cod` (reservada a **(R-revert)**) nem na `submit-row` inferior.                                                                                                 |
| **ITEMNAV-40.4** (**(G-nav.ui-toolbar.4)**) | Margem inferior da faixa suficiente para separar visualmente o grupo de ícones do rótulo «Código Canônico da Natureza de Receita».                                                                                                                   |
| **ITEMNAV-40.5** (**(G-nav.ui-toolbar.5)**) | Em viewport estreita, a faixa pode quebrar linha abaixo do título, mantendo-se **acima** do campo `receita_cod`.                                                                                                                                     |
...
### **ITEMNAV-41** (**(G-nav.ui-controls)**) Botões — símbolos, ordem e estilo
...
| --- | --------------- | --------------------------------------- | --------------------- |
| 1   | `«` ou `<<`     | **ITEMNAV-04** (**(G-nav.prev-level)**) | `core-nav-prev-level` |
| 2   | `<`             | **ITEMNAV-03** (**(G-nav.prev-code)**)  | `core-nav-prev-code`  |
| 3   | `>`             | **ITEMNAV-01** (**(G-nav.next-code)**)  | `core-nav-next-code`  |
| 4   | `»` ou `>>`     | **ITEMNAV-02** (**(G-nav.next-level)**) | `core-nav-next-level` |
...
| Candidato disponível                 | Clique → pipeline **ITEMNAV-42** (**(G-nav.ui-unsaved)**) → **ITEMNAV-43** (**(G-nav.ui-warning-class)**) (se aplicável) → `location.assign(change_url)`. |

Os botões **não** submetem o formulário. Não duplicar handlers em `guardedLinks` do aviso global de saída.

### **ITEMNAV-42** (**(G-nav.ui-unsaved)**) Alterações não guardadas

Paridade com [`spec_itemClassificacao_criar_filho.md`](spec_itemClassificacao_criar_filho.md) (**Alterações não guardadas**):
...
**Sequência de nível na UI:** botões **ITEMNAV-02** (**(G-nav.next-level)**) e **ITEMNAV-04** (**(G-nav.prev-level)**) leem/gravam `structural_nav_nv_target` (URL ou estado equivalente). Botões **ITEMNAV-01** (**(G-nav.next-code)**) e **ITEMNAV-03** (**(G-nav.prev-code)**) **não** incluem o parâmetro e **encerram** a sequência no destino.

### **ITEMNAV-43** (**(G-nav.ui-warning-class)**) Classificação diferente
...
Botões: **Cancelar** | **Continuar**. **Cancelar** mantém a change atual. **Continuar** prossegue o redirecionamento (após **ITEMNAV-42** (**(G-nav.ui-unsaved)**) quando aplicável).

---

## 8. Endpoint (**ITEMNAV-50**)
...
## 10. Protocolo próximo código (**ITEMNAV-01**)
...
## 11. Protocolo próximo nível (**ITEMNAV-02**)
...
- Repetição ascendente (nível `L` de `NV-EDIT − 1` até `1`): **menor** segmento no nível **L** **superior** ao descartado em **COD-EDIT**, com níveis posteriores a **L** em zero canônico no candidato (mesma lógica da etapa-3 de **ITEMNAV-01** (**(G-nav.next-code)**)).
...
### Fase L3 — Descer: um passo de **ITEMNAV-01** (**(G-nav.next-code)**)

Quando L2 produz destino com **NV-EDIT < NV-TARGET**:

- Substituir o destino da L2 por **exactamente um** candidato de **ITEMNAV-01** (**(G-nav.next-code)**) aplicado a **COD-EDIT** actual (mesmas regras de candidatos, **MASK-EDIT** e ordem estrutural).
...
**Nota:** códigos marcados como não navegáveis (ex.: «Classificação exemplo para não navegação») ficam **fora** do conjunto de candidatos **(T-cod.2)**; **ITEMNAV-01** (**(G-nav.next-code)**) na L3 **salta** para o próximo existente.

---

## 12. Protocolo nível anterior (**ITEMNAV-04**)

Espelho de **ITEMNAV-02** (**(G-nav.next-level)**) com **NV-TARGET** e fases L1–L3 invertidas:

| Fase   | **ITEMNAV-02** (**(G-nav.next-level)**)                                            | **ITEMNAV-04** (**(G-nav.prev-level)**)                                             |
| ------ | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **L1** | Irmão **superior** no nível efectivo (`NV-EDIT ≥ NV-TARGET`)                       | Irmão **inferior** no nível efectivo                                                |
| **L2** | Subir: próximo irmão ancestral (etapa-3 de **ITEMNAV-01** (**(G-nav.next-code)**)) | Subir: irmão ancestral anterior (etapa-3 de **ITEMNAV-03** (**(G-nav.prev-code)**)) |
| **L3** | Se **NV-EDIT < NV-TARGET**: **um** passo de **ITEMNAV-01** (**(G-nav.next-code)**) | Se **NV-EDIT > NV-TARGET**: **um** passo de **ITEMNAV-03** (**(G-nav.prev-code)**)  |
...
## 13. Protocolo código anterior (**ITEMNAV-03**)

### etapa-1

Como etapa-1 de **ITEMNAV-01** (**(G-nav.next-code)**), mas varredura do maior para o menor segmento no nível encontrado e, entre candidatos no nível, o de **maior** código completo (candidato estruturalmente **anterior** a **COD-EDIT**).
...
7. Destino com outra `classificacao_id` → modal **ITEMNAV-43** (**(G-nav.ui-warning-class)**); Cancelar permanece; Continuar navega.

...
O modelo distingue dois conceitos fundamentais: a **Série de Classificação (Classification Series)** (um conjunto nomeado de listas estruturadas ao longo do tempo, como a ISIC) e a **Classificação Estatística (Statistical Classification)** propriamente dita (uma lista estruturada específica de categorias mutuamente exclusivas, válida por um período). Uma Classificação Estatística pode ser **linear** ou **hierárquica (hierarchical)**.

São definidos três tipos principais de derivação de uma classificação base:
1.  **Versão (Version):** Uma nova classificação que substitui uma anterior com **mudanças essenciais (essential changes)** (que alteram os limites entre categorias). Versões pertencem à mesma Série de Classificação.
2.  **Variante (Variant):** Uma classificação derivada que modifica a estrutura da classificação base para atender a uma necessidade específica, sem ser uma nova versão. Pode ser uma **variante de extensão (extension variant)** (adiciona níveis inferiores), **variante de agregação (aggregate variant)** (agrupa uma lista linear em hierarquias) ou **variante de reagrupamento (regrouping variant)** (cria novos níveis alternativos, podendo ou não violar a estrutura original).
3.  **Atualização (Update):** Uma substituição que **não envolve mudanças essenciais**, apenas ajustes como notas explicativas ou nomes de categorias.

O modelo também aborda a **classificação flutuante (floating classification)**, que permite mudanças sem a emissão de uma nova classificação, registrando a validade temporal de cada elemento.

...
| **item_classificacao** | Quais são os itens (códigos) de cada nível? | **CÓDIGOS** do ementário, com nome oficial, item mãe na hierarquia, vigência, informações gerenciais, dentre outros. Inclui flags como item gerado e válido atualmente. | `item_id`, `codigo_completo`, `codigo_numerico`, `nivel_id`, `parent_item_id`, `nome_oficial`, `descricao`, `item_gerado`, `valido_atualmente` |
